...
def backtest_v3(df, adx_period=14, st_period=10, st_mult=3.0):
    """V3 BTC Sniper: bounce-confirmation entries at support confluence.
    Waits for dip + reversal candle instead of catching the knife."""
    ema9   = calc_ema(df["close"], 9)
    ema21  = calc_ema(df["close"], 21)
    ema50  = calc_ema(df["close"], 50)
    ...
    for i in range(lookback, len(df)):
        ...
        if in_trade is not None:
            ...
            if in_trade["side"] == "BUY":
                ...
                if lo <= in_trade["stop"]:
                    pnl = in_trade["stop"] - in_trade["entry"]
                    lbl = "SL Hit" if pnl < 0 else ("Breakeven" if pnl < atr_v * 0.05 else "Trail Stop")
                    in_trade.update({"exit_price": in_trade["stop"], "exit_time": bar_dt,
                                     "result": lbl, "pnl": pnl})
                    trades.append(in_trade); in_trade = None; cooldown = 8; continue

                if hi >= in_trade["t1"] and not in_trade.get("t1_hit"):
                    in_trade["t1_hit"] = True
                    in_trade["stop"] = in_trade["entry"] + 0.25 * in_trade["entry_atr"]

                if in_trade.get("t1_hit"):
                    trail = in_trade["peak"] - 0.35 * atr_v
                    if trail > in_trade["stop"]:
                        in_trade["stop"] = trail
                ...
                if in_trade.get("t1_hit"):
                    trail = in_trade["trough"] + 0.35 * atr_v
                    if trail < in_trade["stop"]:
                        in_trade["stop"] = trail
        ...
        # ═══════════════════════════════════════════════════
        # HARD GATES — must ALL be true, otherwise skip bar
        # ═══════════════════════════════════════════════════

        macro_bull = (ema50.iloc[i] > ema200.iloc[i] and
                      ema200.iloc[i] > ema200.iloc[i-5])
        macro_bear = (ema50.iloc[i] < ema200.iloc[i] and
                      ema200.iloc[i] < ema200.iloc[i-5])

        if not macro_bull and not macro_bear:
            continue

        bullish_candle = price > opn.iloc[i]
        bearish_candle = price < opn.iloc[i]
        ...
        # ═══════════════════════════════════════════════════
        # SUPPORT/RESISTANCE CONFLUENCE (count zones nearby)
        # ═══════════════════════════════════════════════════
        ...
        confluence_buy = 0
        if prev_lo <= bb_lower.iloc[i-1] + tol:
            confluence_buy += 1
        if prev_lo <= vwap.iloc[i-1] + tol:
            confluence_buy += 1
        if prev_lo <= ema21.iloc[i-1] + tol:
            confluence_buy += 1
        if prev_lo <= ema50.iloc[i-1] + tol:
            confluence_buy += 1

        confluence_sell = 0
        if prev_hi >= bb_upper.iloc[i-1] - tol:
            confluence_sell += 1
        if prev_hi >= vwap.iloc[i-1] - tol:
            confluence_sell += 1
        if prev_hi >= ema21.iloc[i-1] - tol:
            confluence_sell += 1
        if prev_hi >= ema50.iloc[i-1] - tol:
            confluence_sell += 1

        # ═══════════════════════════════════════════════════
        # CONFIRMATION CONDITIONS (score-based)
        # ═══════════════════════════════════════════════════
        rsi_was_low  = any(rsi.iloc[i-j] < 40 for j in range(1, 4))
        rsi_was_high = any(rsi.iloc[i-j] > 60 for j in range(1, 4))
        rsi_recovering = rsi.iloc[i] > rsi.iloc[i-1]
        rsi_declining  = rsi.iloc[i] < rsi.iloc[i-1]

        macd_turn_buy  = macd_hist.iloc[i] > macd_hist.iloc[i-1]
        macd_turn_sell = macd_hist.iloc[i] < macd_hist.iloc[i-1]
        ...
        candle_body = abs(price - opn.iloc[i])
        candle_range = hi - lo
        strong_body = candle_body > candle_range * 0.4 if candle_range > 0 else False
...
with tab4:
    ...
    with bt_c2:
        bt_tf = st.selectbox("Timeframe", list(BINANCE_INTERVALS.keys()),
                             index=1, key="bt_tf")
    with bt_c3:
        default_min = 7 if "V3" in bt_strat else (5 if "V1" in bt_strat else 4)
        bt_min = st.slider("Min Score", 4, 10, default_min, key="bt_min",
    ...
    if st.button("🚀 Run Backtest", type="primary", key="run_bt"):
        ...
        with st.spinner(f"Fetching {bt_sym} data & running backtest…"):
            try:
                ...
                if not trades:
                    ...
                    max_conds = 10 if strat_choice == "V3" else 7
        ...
        with cmp1:
            ...
            **How:** Wait for bounce at support, not the dip.

            | Feature | Why it works |
            |---------|-------------|
            | EMA 50>200 + slope | Hard gate: macro trend |
            | Reversal candle | Green after red = bounce |
            | 2+ support confluence | BB + VWAP + EMA zones |
            | RSI was <40 recently | Was oversold, recovering |
            | MACD turning up | Momentum shifting back |
            | ADX > 20 | Trending market |
            | Strong candle body | Real conviction, not wick |
            | SL = 1× ATR | Safe (entry after bounce) |
            | T1 = 0.8× ATR → lock | Quick profit guaranteed |
            | Trail = 0.35× ATR | Ultra-tight, keep gains |
            | 8-bar cooldown | Avoid chop re-entry |
            | 6/9 score needed | Selective + confirmed |
            """)
...
# ───────────────────── TAB 5 — PAPER TRADE (ALPACA) ─────
with tab5:
    st.markdown("## 📝 Paper Trade — BTC Sniper V3 via Alpaca")
    st.markdown("""<div style="background:#0d1b2a;border:1px solid #2962FF;border-radius:8px;padding:12px 16px;margin-bottom:16px">
    <span style="color:#c9d1d9">Live paper trading on <b>Alpaca</b> using the V3 BTC Sniper strategy.
    When a signal fires, a real paper order is placed on your Alpaca account. <b>No real money — paper trading only.</b></span>
    </div>""", unsafe_allow_html=True)

    with st.sidebar:
        st.markdown("---")
        st.markdown("### 🔑 Alpaca Paper Trading")
        alpaca_key = st.text_input("API Key", type="password", key="alpaca_key",
                                   help="From https://app.alpaca.markets/paper/dashboard/overview")
        ...
        if alpaca_key and alpaca_secret and ALPACA_AVAILABLE:
            try:
                ...
                st.success(f"Connected! Balance: ${float(acct.cash):,.2f}")
            except Exception as e:
    ...
    if not ALPACA_AVAILABLE:
        ...
        st.warning("👈 Enter your Alpaca Paper Trading API keys in the sidebar to connect.")
        st.markdown("""
        ...
        5. Copy the **API Key** and **Secret Key** into the sidebar
        """)
    else:
        acct = alpaca_client.get_account()
        ...
        a3.metric("🔋 Buying Power", f"${buying_power:,.2f}")
        pnl_today = float(acct.equity) - float(acct.last_equity)
        a4.metric("📈 Today's P&L", f"${pnl_today:+,.2f}",
                  f"{pnl_today/float(acct.last_equity)*100:+.2f}%" if float(acct.last_equity) > 0 else None)
        ...
        if btc_pos:
            unr = float(btc_pos.unrealized_pl)
            unr_pct = float(btc_pos.unrealized_plpc) * 100
            qty = float(btc_pos.qty)
            side_str = btc_pos.side.value.upper()
            ...
            <h3 style="color:{color};margin:0">{"🟢" if side_str == "LONG" else "🔴"} ACTIVE POSITION — {side_str} BTC</h3>
            <table style="width:100%;color:#c9d1d9;margin-top:12px;font-size:1rem">
            <tr><td><b>Qty:</b> {qty:.6f} BTC</td>
                <td><b>Entry:</b> ${float(btc_pos.avg_entry_price):,.2f}</td>
                <td><b>Current:</b> ${float(btc_pos.current_price):,.2f}</td>
                <td><b style="color:{color}">P&L: ${unr:,.2f} ({unr_pct:+.1f}%)</b></td></tr>
            <tr><td><b>Market Value:</b> ${float(btc_pos.market_value):,.2f}</td>
                <td><b>Cost Basis:</b> ${float(btc_pos.cost_basis):,.2f}</td>
                <td colspan="2"></td></tr>
            </table>
            </div>""", unsafe_allow_html=True)
            ...
            with close_col1:
                if st.button("🚪 Close Position (Market)", key="close_pos",
                             type="secondary", use_container_width=True):
                    try:
                        alpaca_client.close_position("BTC/USD")
        ...
        with pt_c1:
            pt_risk_pct = st.slider("Risk per Trade (%)", 1, 10, 2, key="pt_risk_alp",
                                    help="% of equity risked per trade")
        with pt_c2:
            pt_tf = st.selectbox("Timeframe", ["1 hour", "15 min"], key="pt_tf_alp")

        scan_btn = st.button("🔍 Scan & Auto-Trade", type="primary",
                             key="pt_scan_alp", use_container_width=True)

        if scan_btn:
            interval_code, limit = BINANCE_INTERVALS[pt_tf]
            with st.spinner("Scanning BTC for V3 signal…"):
                try:
                    ...
                    _, bb_upper_pt, bb_lower_pt = calc_bbands(df_pt["close"], 20, 2.0)
                    ...
                    buy_rev = (price > opn_v and df_pt["close"].iloc[i-1] < df_pt["open"].iloc[i-1]
                               and macro_bull and close_pos_b > 0.55)
                    sell_rev = (price < opn_v and df_pt["close"].iloc[i-1] > df_pt["open"].iloc[i-1]
                    ...
                    if buy_rev or sell_rev:
                        ...
                        conf_buy = sum([prev_lo <= bb_lower_pt.iloc[i-1] + tol,
                                        prev_lo <= vwap_pt.iloc[i-1] + tol,
                                        prev_lo <= ema21_pt.iloc[i-1] + tol,
                                        prev_lo <= ema50_pt.iloc[i-1] + tol])
                        conf_sell = sum([prev_hi >= bb_upper_pt.iloc[i-1] - tol,
                                         prev_hi >= vwap_pt.iloc[i-1] - tol,
                                         prev_hi >= ema21_pt.iloc[i-1] - tol,
                                         prev_hi >= ema50_pt.iloc[i-1] - tol])

                        rsi_low = any(rsi_pt.iloc[i-j] < 40 for j in range(1, 4))
                        rsi_high = any(rsi_pt.iloc[i-j] > 60 for j in range(1, 4))
                        ...
                        if buy_rev:
                            sc = sum([conf_buy >= 2, rsi_low, rsi_pt.iloc[i] > rsi_pt.iloc[i-1],
                                      macd_up, adx_ok, pdi_pt.iloc[i] > mdi_pt.iloc[i],
                                      dr_pt.iloc[i] == -1, vol_ok, strong_b])
                            if sc >= 6:
                                signal_found = "BUY"
                        elif sell_rev:
                            sc = sum([conf_sell >= 2, rsi_high, rsi_pt.iloc[i] < rsi_pt.iloc[i-1],
                                      macd_dn, adx_ok, mdi_pt.iloc[i] > pdi_pt.iloc[i],
                                      dr_pt.iloc[i] == 1, vol_ok, strong_b])
                            if sc >= 6:
                                signal_found = "SELL"

                    ind_col1, ind_col2, ind_col3, ind_col4 = st.columns(4)
                    trend_label = "Bullish" if ema50_pt.iloc[i] > ema200_pt.iloc[i] else "Bearish"
                    ind_col1.metric("Trend", trend_label)
                    ind_col2.metric("ADX", f"{adx_pt.iloc[i]:.1f}")
                    ind_col3.metric("RSI", f"{rsi_pt.iloc[i]:.1f}")
                    ind_col4.metric("ATR", f"${atr_v:,.2f}")

                    if signal_found and not btc_pos:
                        risk_amt = equity * pt_risk_pct / 100
                        sl_dist = 1.2 * atr_v
                        qty_btc = round(risk_amt / sl_dist, 6)
                        qty_btc = max(qty_btc, 0.0001)

                        side = OrderSide.BUY if signal_found == "BUY" else OrderSide.SELL
                        ...
                        try:
                            ...
                            sl_price = live_price - 1.2 * atr_v if signal_found == "BUY" else live_price + 1.2 * atr_v
                            t1_price = live_price + 0.6 * atr_v if signal_found == "BUY" else live_price - 0.6 * atr_v
                            t2_price = live_price + 1.5 * atr_v if signal_found == "BUY" else live_price - 1.5 * atr_v

                            st.success(f"🎯 **{signal_found} ORDER PLACED!** {qty_btc:.6f} BTC at ~${live_price:,.2f}")
                            st.markdown(f"""<div style="background:#0d2b1a;border:2px solid #00e676;border-radius:12px;padding:16px;margin:10px 0">
                            <h4 style="color:#00e676;margin:0">Order Placed on Alpaca (Paper)</h4>
                            <table style="width:100%;color:#c9d1d9;margin-top:8px">
                            <tr><td><b>Side:</b> {signal_found}</td><td><b>Qty:</b> {qty_btc:.6f} BTC</td><td><b>Score:</b> {sc}/9</td></tr>
                            <tr><td><b>Stop Loss:</b> <span style="color:#ff5252">${sl_price:,.2f}</span></td>
                                <td><b>T1:</b> ${t1_price:,.2f}</td><td><b>T2:</b> ${t2_price:,.2f}</td></tr>
                            <tr><td colspan="3" style="color:#ffd700;padding-top:8px"><b>⚠️ Set stop-loss manually on Alpaca dashboard at ${sl_price:,.2f}</b></td></tr>
                            </table>
                            </div>""", unsafe_allow_html=True)
                        except Exception as e:
                            st.error(f"Order failed: {e}")

                    elif signal_found and btc_pos:
                        st.warning(f"V3 signal: **{signal_found}** (Score {sc}/9) — but you already have an open BTC position. Close it first.")
                    else:
                        st.markdown("""<div style="background:#1a1a2e;border:1px solid #555;border-radius:12px;padding:20px;margin:10px 0">
                        <h3 style="color:#ffd700;margin:0">⏳ No V3 Signal Right Now</h3>
                        <p style="color:#c9d1d9;margin-top:8px">The V3 Sniper is waiting for a dip to support + bounce confirmation.
                        Scan again in 1 hour when the next candle closes.</p>
                        </div>""", unsafe_allow_html=True)
        ...
        st.markdown("---")
        orders = alpaca_client.get_orders(filter=None)
        recent = [o for o in orders if o.symbol in ("BTC/USD", "BTCUSD")][:10]
        if recent:
            st.markdown("### 📋 Recent Alpaca Orders")
            order_data = []
            for o in recent:
                order_data.append({
                    ...
                    "Filled Avg": f"${float(o.filled_avg_price):,.2f}" if o.filled_avg_price else "—",
                    "Type": o.type.value,
                })
        ...
        <strong style="color:#ffd700">How Alpaca Paper Trading Works:</strong>
        <ol style="color:#c9d1d9;margin:8px 0">
        <li>Connect with your Alpaca Paper Trading API keys (free account)</li>
        <li>Click <b>Scan & Auto-Trade</b> — runs V3 on latest BTC candles</li>
        <li>If V3 conditions are met, a <b>real paper order</b> is placed on Alpaca</li>
        <li>Position shows up on your <b>Alpaca dashboard</b> with live P&L</li>
        <li><b>Set stop-loss manually</b> on Alpaca at the suggested SL price</li>
        <li>Click <b>Close Position</b> when T1/T2 is reached, or let the trail manage it</li>
        <li>Track performance across your Alpaca portfolio history</li>
        </ol>
        <p style="color:#888;margin-top:8px">💡 Alpaca gives you $100,000 virtual cash. Crypto trades 24/7.</p>
        </div>""", unsafe_allow_html=True)
